...
pub fn load(app: &AppHandle) -> AppResult<Settings> {
    ...
    let mut settings = if path.exists() {
        ...
        // A settings file damaged by a crash or hand editing should not brick
        // the app: fall back to defaults rather than refusing to start.
        serde_json::from_str(&json).unwrap_or_default()
    } else {
        Settings::default()
    };

    // The token is minted on the first load that lacks one — a new install or
    // a file from before the server existed — and saved so it stays put.
