...
export const api = {
  ...
  openModProject: (modFilePath: string) =>
    call<ModProject>("open_mod_project", { modFilePath }),

  readModProject: (modFilePath: string) =>
    call<ModProject>("read_mod_project", { modFilePath }),

  forgetRecentProject: (folderPath: string) =>
    call<Settings>("forget_recent_project", { folderPath }),

  scanProjectFiles: (folderPath: string) =>
    call<ScanResult>("scan_project_files", { folderPath }),

  scanDirectory: (rootPath: string, maxFiles?: number) =>
    call<ScanResult>("scan_directory", { rootPath, maxFiles }),
  ...
  addFocus: (path: string, treeId: string, focus: FocusUpdate) =>
    call<FocusFile>("add_focus", { path, treeId, focus }),

  deleteFocus: (path: string, focusId: string) =>
    call<FocusFile>("delete_focus", { path, focusId }),

  listLocalisationFiles: (folderPath: string) =>
    call<LocalisationFileInfo[]>("list_localisation_files", { folderPath }),

  readLocalisationFile: (path: string) =>
    call<LocalisationFile>("read_localisation_file", { path }),
