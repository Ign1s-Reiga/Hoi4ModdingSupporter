...
export default function AssetsPage() {
  ...
  const [preview, setPreview] = React.useState<{ path: string; url: string | null } | null>(null);
  ...
  const gameScanKey = source === 'game' && gameRoot ? `${gameRoot.replace(/[\\/]+$/, '')}/${area.prefixes[0]}` : null;
  ...
  const files = React.useMemo(() => {
    const pool =
      source === 'mod'
        ? (scan?.files ?? []).filter((file) => area.prefixes.some((prefix) => isInFolder(file.relativePath, prefix)))
        : gameFiles;

    const needle = search.trim().toLowerCase();
    return needle ? pool.filter((file) => file.relativePath.toLowerCase().includes(needle)) : pool;
            ...
            <>
                ...
                <div className='grid grid-cols-[repeat(auto-fill,minmax(7rem,1fr))] gap-2'>
                  {images.slice(0, MAX_TILES).map((file) => (
                    ...
                    >
                      ...
                      <span className='truncate px-1.5 py-1 text-[0.6875rem] text-muted'>{file.name}</span>
                    </button>
                  ))}
                </div>
              ) : null}
              ...
              {files.length > MAX_TILES ? (
                <p className='mt-3 text-xs text-muted'>
                  Showing the first {MAX_TILES} entries of {files.length}. Narrow the filter to see more.
                </p>
              ) : null}
            </>
          )}
            ...
            <div className='flex flex-col gap-3'>
              {selected.kind === 'image' ? (
                <div className='flex min-h-40 items-center justify-center rounded-md border border-border bg-surface-sunken p-2'>
                  {currentPreview?.url ? (
                    // Data URL from the backend, so next/image cannot help here.
                    // oxlint-disable-next-line next/no-img-element
                    <img src={currentPreview.url} alt='' className='max-h-64 max-w-full object-contain' />
                  ) : currentPreview ? (
              ...
              <Button
                size='sm'
                onClick={() => {
                  void revealItemInDir(selected.fullPath).catch((error) => setError(describeError(error)));
                }}
              >
                <ExternalLink />
                Show in Explorer
